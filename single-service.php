...
$categories = wp_get_post_terms($post->ID, 'service-category', 'fields=ids');
$tags = wp_get_post_terms($post->ID, 'service-tag');
$pricing_options = wp_get_post_terms($post->ID, 'service-pricing-option');

$related_services = array();

if ($related_services = get_post_meta($post->ID, '_related_services', true)) {
					...
					<?php foreach ($related_services as $related_service) : ?>
						<div class="item">
							<div class="product-box">
								<?php if (has_post_thumbnail($related_service)) : ?>
									<div class="product-box__image">
										<a href="<?php echo get_permalink($related_service); ?>">
											<?php echo get_the_post_thumbnail($related_service, '480x360'); ?>
										</a>
									</div>
								<?php endif; ?>
								<div class="product-box__info">
									<h3 class="product-box__title">
										<a href="<?php echo get_permalink($related_service); ?>">
											<?php echo get_the_title($related_service); ?>
										</a>
									</h3>
									<div class="product-box__text">
										<p><?php echo $related_service->post_excerpt ? $related_service->post_excerpt : mb_strimwidth($related_service->post_content, 0, 80, '...'); ?></p>
									</div>
								</div>
								<?php $amount = get_post_meta($related_service->ID, '_amount', true); ?>
								<?php if ($amount != '') : ?>
									<div class="product-box__price">
										...
										<?php else : ?>
											<?php echo is_numeric($amount) ? __('Price from', 'wb') : ''; ?><div class="price"><?php echo get_post_meta($related_service->ID, '_currency', true); ?><?php echo $amount; ?></div>
										<?php endif; ?>
									</div>
								<?php endif; ?>
							</div>
