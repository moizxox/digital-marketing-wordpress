...
$categories = wp_get_post_terms($post->ID, 'course-category', 'fields=ids');
$tags = wp_get_post_terms($post->ID, 'course-tag');
$pricing_options = wp_get_post_terms($post->ID, 'course-pricing-option');

$related_courses = array();

if ($related_courses = get_post_meta($post->ID, '_related_courses', true)) {
					...
					<?php foreach ($related_courses as $related_course) : ?>
						<div class="item">
							<div class="product-box">
								<?php if (has_post_thumbnail($related_course)) : ?>
									<div class="product-box__image">
										<a href="<?php echo get_permalink($related_course); ?>">
											<?php echo get_the_post_thumbnail($related_course, '480x360'); ?>
										</a>
									</div>
								<?php endif; ?>
								<div class="product-box__info">
									<h3 class="product-box__title">
										<a href="<?php echo get_permalink($related_course); ?>">
											<?php echo get_the_title($related_course); ?>
										</a>
									</h3>
									<div class="product-box__text">
										<p><?php echo $related_course->post_excerpt ? $related_course->post_excerpt : mb_strimwidth($related_course->post_content, 0, 80, '...'); ?></p>
									</div>
								</div>
								<?php $amount = get_post_meta($related_course->ID, '_amount', true); ?>
								<?php if ($amount != '') : ?>
									<div class="product-box__price">
										...
										<?php else : ?>
											<?php echo is_numeric($amount) ? __('Price from', 'wb') : ''; ?><div class="price"><?php echo get_post_meta($related_course->ID, '_currency', true); ?><?php echo $amount; ?></div>
										<?php endif; ?>
									</div>
								<?php endif; ?>
							</div>
